-- ============================================================
-- Migration 040: Modul-spezifische CTAs auf Unterseiten
--
-- Alle 19 Modul-Detailseiten (A1-A5 / B1-B5 / C1-C6 / D1-D3) plus
-- /loesungen-Hauptseite bekommen Conversion-optimierte CTAs:
--   - Problemorientierte Headline (statt "Interesse an Modul XY?")
--   - Konkretes Versprechen, was im 30-Min-Gespraech passiert
--   - Aktiver Button-Text (statt "Erstgespraech buchen")
--   - Topic-Parameter im href -> Tag im Kontaktformular vorausgewaehlt
--
-- Idempotent: setzt den CTA-Block (block_type='cta') vollstaendig neu.
-- Bei mehreren CTA-Bloecken auf einer Seite wird der mit dem
-- niedrigsten sort_order angepasst (= primaerer CTA).
-- ============================================================

CREATE OR REPLACE FUNCTION xkmu_set_module_cta(
  p_slug text, p_headline text, p_description text,
...
BEGIN
  ...
  -- URL-Encode Spaces / Sonderzeichen im Topic-Parameter
...
-- ─── KI-Beratung A1-A5 ─────────────────────────────────────────────
SELECT xkmu_set_module_cta('/ki-beratung/a1',
...
SELECT xkmu_set_module_cta('/ki-beratung/a2',
...
SELECT xkmu_set_module_cta('/ki-beratung/a3',
...
SELECT xkmu_set_module_cta('/ki-beratung/a4',
...
SELECT xkmu_set_module_cta('/ki-beratung/a5',
...
-- ─── IT-Beratung B1-B5 ─────────────────────────────────────────────
SELECT xkmu_set_module_cta('/it-beratung/b1',
...
SELECT xkmu_set_module_cta('/it-beratung/b2',
...
SELECT xkmu_set_module_cta('/it-beratung/b3',
...
SELECT xkmu_set_module_cta('/it-beratung/b4',
...
SELECT xkmu_set_module_cta('/it-beratung/b5',
...
-- ─── Cybersecurity C1-C6 ───────────────────────────────────────────
SELECT xkmu_set_module_cta('/cybersecurity/c1',
...
SELECT xkmu_set_module_cta('/cybersecurity/c2',
...
SELECT xkmu_set_module_cta('/cybersecurity/c3',
...
SELECT xkmu_set_module_cta('/cybersecurity/c4',
...
SELECT xkmu_set_module_cta('/cybersecurity/c5',
...
SELECT xkmu_set_module_cta('/cybersecurity/c6',
...
-- ─── Kombinations-Module D1-D3 + /loesungen ─────────────────────────
SELECT xkmu_set_module_cta('/loesungen/d1',
...
SELECT xkmu_set_module_cta('/loesungen/d2',
...
SELECT xkmu_set_module_cta('/loesungen/d3',
...
-- /loesungen-Hauptseite (nutzt pillarBlocks; keine backHref-Buttons → wir lassen
-- den outline-Button weg, indem wir auf eine eigene Befehlsversion fallen):
DO $$
DECLARE
  v_page_id uuid;
  v_cta_id uuid;
BEGIN
  SELECT id INTO v_page_id FROM cms_pages WHERE slug = '/loesungen';
  IF v_page_id IS NULL THEN RETURN; END IF;
  ...
  IF v_cta_id IS NOT NULL THEN
    UPDATE cms_blocks SET content = '{
      "headline": "Drei Säulen, ein Ansprechpartner — passt das zu Ihnen?",
      "description": "Wenn KI, IT und Cybersecurity bei Ihnen heute getrennt laufen oder unkoordiniert wachsen, lohnt ein Erstgespräch über unsere Kombinations-Module. 30 Minuten, kein Verkaufsdruck.",
      "buttons": [
        {"label": "Kombi-Modul besprechen", "href": "/kontakt?interesse=Kombinations-Modul", "variant": "default"}
      ]
    }'::jsonb
    WHERE id = v_cta_id;
  END IF;
...
-- ─── Hilfsprozedur droppen ──────────────────────────────────────────
DROP FUNCTION xkmu_set_module_cta(text, text, text, text, text, text);

-- ─── Published-Snapshot fuer alle 20 Seiten neu aufbauen ────────────
