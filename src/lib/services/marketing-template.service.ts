...
export const MarketingTemplateService = {
  async list(tenantId: string, filters: TemplateFilters = {}) {
    ...
    const conditions = [eq(marketingTemplates.tenantId, tenantId)]
  ...
  async getById(tenantId: string, id: string): Promise<MarketingTemplate | null> {
    const [template] = await db
      .select()
      .from(marketingTemplates)
      .where(and(eq(marketingTemplates.tenantId, tenantId), eq(marketingTemplates.id, id)))
      .limit(1)
    return template ?? null
  },

  async getDefault(tenantId: string, type: string): Promise<MarketingTemplate | null> {
    const [template] = await db
      .select()
      .from(marketingTemplates)
      .where(and(
        eq(marketingTemplates.tenantId, tenantId),
  ...
  async create(tenantId: string, data: CreateTemplateInput): Promise<MarketingTemplate> {
    const [template] = await db
  ...
  async update(tenantId: string, id: string, data: UpdateTemplateInput): Promise<MarketingTemplate | null> {
