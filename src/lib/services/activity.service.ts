...
export const ActivityService = {
  async create(tenantId: string, data: CreateActivityInput, userId?: string | null): Promise<Activity> {
  ...
  async getById(tenantId: string, activityId: string): Promise<Activity | null> {
    const [activity] = await db
      .select()
      .from(activities)
  ...
  async listByLead(tenantId: string, leadId: string, filters: Pick<ActivityFilters, 'page' | 'limit'> = {}) {
    const { page = 1, limit = 50 } = filters
    const offset = (page - 1) * limit

    const whereClause = and(eq(activities.tenantId, tenantId), eq(activities.leadId, leadId))
  ...
  async listByCompany(tenantId: string, companyId: string, filters: Pick<ActivityFilters, 'page' | 'limit'> = {}) {
    const { page = 1, limit = 50 } = filters
    const offset = (page - 1) * limit

    const whereClause = and(eq(activities.tenantId, tenantId), eq(activities.companyId, companyId))
