...
describe('MetaProvider.publish', () => {
  it('routes facebook post to publishToFacebookPage with decrypted token + content', async () => {
  ...
  it('routes instagram post to publishToInstagram with caption + image url', async () => {
    const dbMock = setupDbMock()
    dbMock.selectMock.mockResolvedValue([{
  ...
  it('rejects unsupported platform with throw (programmer error)', async () => {
    const { MetaProvider } = await import('@/lib/services/social/meta-provider')
    await expect(MetaProvider.publish({ id: 'pst4', platform: 'x', content: 'x', imageUrl: null } as any))
