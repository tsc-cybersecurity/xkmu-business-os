...
export default function ServicesPage() {
  ...
  const handleDelete = async (service: Service) => {
    if (!confirm(`Dienstleistung "${service.name}" wirklich löschen?`)) return
    try {
      const res = await fetch(`/api/v1/products/${service.id}`, { method: 'DELETE' })
      ...
      logger.error('Failed to delete service', error, { module: 'CatalogServicesPage' })
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-1">
                          <Button asChild variant="ghost" size="icon" className="h-8 w-8" title="Bearbeiten">
                            <Link href={`/intern/catalog/services/${service.id}`}>
