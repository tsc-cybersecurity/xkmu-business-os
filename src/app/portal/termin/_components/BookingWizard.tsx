...
export function BookingWizard({ onBooked }: { onBooked?: () => void }) {
  ...
  useEffect(() => {
    if (!selStaff || !selSlotType) return
    const monthParam = `${viewYear}-${String(viewMonth + 1).padStart(2, '0')}`
    let cancelled = false
    setCountsLoaded(false)
    fetch(`/api/portal/termin/availability/month?userId=${selStaff.id}&slotTypeId=${selSlotType.id}&month=${monthParam}`)
      .then(r => r.json())
      .then(d => {
        if (cancelled) return
  ...
  }, [viewYear, viewMonth, selStaff, selSlotType])
