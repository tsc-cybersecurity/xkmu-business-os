...
export async function POST(request: NextRequest, ctx: Ctx) {
  return withPermission(request, 'users', 'update', async (auth) => {
    ...
    try {
      ...
      const cert = await CourseCertificateService.reject(certificateId, auth.userId, v.data.reviewComment)
      ...
      logger.error('Certificate reject failed', err, { module: 'CertificateAdminAPI' })
