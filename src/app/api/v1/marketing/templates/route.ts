...
import { createMarketingTemplateSchema,
  validateAndParse,
  formatZodErrors,
...
export async function GET(request: NextRequest) {
  return withPermission(request, 'marketing', 'read', async (auth) => {
    ...
    const result = await MarketingTemplateService.list({
      ...pagination,
      type,
    })
...
export async function POST(request: NextRequest) {
  return withPermission(request, 'marketing', 'create', async (auth) => {
    try {
      ...
      const template = await MarketingTemplateService.create(validation.data)
