  ...
  return withPermission(request, 'settings', 'read', async (auth) => {
    try {
      ...
      const [{ newLeads }] = await db.select({ newLeads: count() }).from(leads)
        .where(and(eq(leads.tenantId, TENANT_ID), gte(leads.createdAt, from), lte(leads.createdAt, to)))

      // Won leads
      const [{ wonLeads }] = await db.select({ wonLeads: count() }).from(leads)
        .where(and(eq(leads.tenantId, TENANT_ID), eq(leads.status, 'won'), gte(leads.updatedAt, from), lte(leads.updatedAt, to)))

      // Revenue (paid invoices)
      const [{ revenue }] = await db.select({ revenue: sum(documents.total) }).from(documents)
        .where(and(eq(documents.tenantId, TENANT_ID), eq(documents.type, 'invoice'), eq(documents.paymentStatus, 'paid'), gte(documents.paidAt, from), lte(documents.paidAt, to)))

      // Open invoices
      const [{ openInvoices }] = await db.select({ openInvoices: count() }).from(documents)
        .where(and(eq(documents.tenantId, TENANT_ID), eq(documents.type, 'invoice'), eq(documents.paymentStatus, 'unpaid')))

      // Overdue invoices
      const [{ overdueInvoices }] = await db.select({ overdueInvoices: count() }).from(documents)
        .where(and(eq(documents.tenantId, TENANT_ID), eq(documents.type, 'invoice'), eq(documents.paymentStatus, 'overdue')))
